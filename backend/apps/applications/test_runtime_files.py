...
def test_runtime_folder_browser_allows_paths_outside_configured_roots(tmp_path, settings):
    ...
    assert response.status_code == 200
    assert response.data["path"] == str(outside.resolve())


def test_runtime_folder_browser_can_restore_root_restrictions(tmp_path, settings):
    allowed = tmp_path / "allowed"
    outside = tmp_path / "outside"
    allowed.mkdir()
    outside.mkdir()
