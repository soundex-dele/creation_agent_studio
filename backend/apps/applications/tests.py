...
from modules.catalog.models import (
    AgentDraft, ApplicationDeployment, ApplicationDraft,
    ChatApplicationRevision, DeploymentEnvironment,
)
...
class ChatApplicationBoundaryTest(TestCase):
    ...
    def _create_chat(self, *, slug='chat-test'):
        ...
        return response, Application.objects.get(slug=slug)
    ...
    def test_non_chat_runtime_has_no_chat_members(self):
        response = self.client.post('/api/apps/', {
            'category': self.app_category.id, 'name': 'Task', 'slug': 'task-test',
            'description': 'Task app', 'kind': 'task',
            'renderer_key': 'generic-task', 'executor_key': 'task-executor',
        }, format='json', **self.headers)
        self.assertEqual(response.status_code, 201, response.data)
        self.assertNotIn('agent_bindings', response.data)
        self.assertNotIn('skill_bindings', response.data)
        self.assertNotIn('guided_prompts', response.data)
        self.assertNotIn('chat_profile', response.data)
    ...
    def test_compose_and_conversation_use_chat_definition(self):
        ...
        self.assertIsNone(conversation.workflow_step_run_id)
    ...
    def test_chat_update_replaces_the_single_draft_source(self):
        ...
        response = self.client.patch(
            ...
            {'chat_profile': {
                **application.draft.content['chat_profile'],
    ...
    def test_workflow_conversation_derives_application_without_duplicate_fk(self):
        _response, application = self._create_chat()
        revision = publish_application(
            application=application, actor=self.user,
            expected_draft_version=application.draft.version)
        ...
        workflow = self.client.post('/api/workflows/', {
            'name': 'Chat flow',
            'steps': [{'application_id': application.id, 'order': 0,
                       'name': 'Chat', 'config': {}}],
        }, format='json', **self.headers)
        self.assertEqual(workflow.status_code, 201, workflow.data)
        run = self.client.post(
            f"/api/workflows/{workflow.data['id']}/start/", {},
            format='json', **self.headers)
        self.assertEqual(run.status_code, 201, run.data)
        step_run_id = run.data['step_runs'][0]['id']
        created = self.client.post('/api/conversations/', {
            'workflow_step_run_id': step_run_id,
        }, format='json', **self.headers)
        self.assertEqual(created.status_code, 201, created.data)
        conversation = self.user.conversations.get(id=created.data['id'])
        self.assertIsNone(conversation.chat_application_id)
        self.assertEqual(conversation.application_id, application.id)
        self.assertEqual(
            conversation.workflow_step_run.application_revision_id,
            revision.id)
    ...
    def test_rejects_private_agent_from_another_tenant(self):
        ...
        response = self.client.post('/api/apps/', {
            'category': self.app_category.id, 'name': 'Rejected',
            'slug': 'rejected', 'description': 'Rejected', 'kind': 'chat',
            'renderer_key': 'chat', 'executor_key': 'agent-chat',
            'agent_bindings': [{
