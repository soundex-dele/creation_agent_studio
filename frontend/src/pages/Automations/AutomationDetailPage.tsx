...
export default function AutomationDetailPage() {
  ...
  return <div className="automation-detail animate-fade-in">
    <div className="automations-hero"><div><Space><h1 className="page-title">{automation.name}</h1><Tag>{automation.status}</Tag></Space><p className="page-subtitle">{automation.description || '暂无描述'}</p></div><Space><Button onClick={() => navigate('/automations')}>返回</Button>{canEdit && <Button onClick={() => navigate(`/automations/${automation.id}/edit`)}>编辑</Button>}</Space></div>
