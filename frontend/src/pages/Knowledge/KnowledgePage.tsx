...
export default function KnowledgePage() {
  ...
  return <div className="knowledge-page">
    ...
    <Modal title={baseForm.getFieldValue('id') ? '知识库设置' : '新建知识库'} open={baseModal} onCancel={() => setBaseModal(false)} onOk={() => void saveBase()} width={680}><Form layout="vertical" form={baseForm}><Form.Item name="id" hidden><Input /></Form.Item><Form.Item name="name" label="名称" rules={[{ required: true }]}><Input /></Form.Item><Form.Item name="description" label="描述"><Input.TextArea rows={3} /></Form.Item><Space align="start"><Form.Item name="chunk_size" label="分块 Token 数"><InputNumber min={100} max={1500} /></Form.Item><Form.Item name="chunk_overlap" label="重叠 Token 数"><InputNumber min={0} max={749} /></Form.Item></Space>{canAdmin && <><Form.Item name="embedding_provider" label="嵌入供应商名称"><Input /></Form.Item><Form.Item name="embedding_model" label="嵌入模型"><Input /></Form.Item><Form.Item name="answer_provider" label="回答供应商名称"><Input /></Form.Item><Form.Item name="answer_model" label="回答模型"><Input /></Form.Item></>}</Form></Modal>
