...
export default function AgentLifecyclePanel() {
  ...
  const loadDetails = useCallback(async () => { if (!agentId) { setVersions([]); setDeployment(null); return; } const [v, d] = await Promise.all([api.get(`/agents/${agentId}/versions/`), api.get(`/agents/${agentId}/deployment/`)]); setVersions(v as Row[]); setDeployment(d as Row | null); }, [agentId]);
  ...
  return <div className="enterprise-subpanel">
    <Alert type="info" showIcon message="发布流程" description="开发者创建并发布不可变版本，组织管理员在当前实例激活版本；跨环境晋级由外部发布流程完成。" />
    <Card title="智能体版本管理" extra={<Space><Select style={{ width: 260 }} placeholder="选择组织智能体" value={agentId} onChange={setAgentId} options={agents.map(a => ({ value: a.id, label: a.name }))} /><Button type="primary" disabled={!agentId} onClick={() => { versionForm.resetFields(); setVersionOpen(true); }}>新建版本</Button></Space>}>
      {!agents.length ? <Alert type="warning" message="当前组织暂无智能体，请先通过智能体 API 创建组织智能体。" /> : <Table rowKey="id" dataSource={versions} scroll={{ x: 1100 }} columns={[
        ...
        { title: '操作', width: 520, render: (_, v: Row) => <Space wrap>{v.status === 'draft' && <Button size="small" onClick={() => act(v, 'submit')}>送审</Button>}{v.status === 'in_review' && <><Button size="small" type="primary" onClick={() => act(v, 'review', { decision: 'approved' })}>批准</Button><Button danger size="small" onClick={() => act(v, 'review', { decision: 'rejected' })}>拒绝</Button></>}<Button size="small" type="primary" onClick={() => deploy(v)}>激活版本</Button></Space> },
      ]} />}
    </Card>
    <Card title="当前激活部署" style={{ marginTop: 16 }}><Table rowKey="id" pagination={false} dataSource={deployment ? [deployment] : []} columns={[{ title: '当前 Revision', dataIndex: 'revision_id' }, { title: '部署版本', dataIndex: 'version' }, { title: '更新时间', dataIndex: 'updated_at' }, { title: '操作', render: (_, d: Row) => <Button size="small" disabled={!d.previous_revision_id} onClick={() => rollback()}>回滚</Button> }]} /></Card>
